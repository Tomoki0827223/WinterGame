...
void EnemySkydome5::Initialize(Model* model, ViewProjection* viewProjection, const Vector3& pos) {
	assert(model);
	model_ = model;
...
void EnemySkydome5::Update() {
	Fire();

	// 弾更新
	for (EnemySkydomeBullet5* bullet : bullets5_) {
		bullet->Update();
	}

	// デスフラグの立った弾を削除
	bullets5_.remove_if([](EnemySkydomeBullet5* bullet) {
		if (bullet->IsDead()) {
			delete bullet;
			return true;
		}
...
void EnemySkydome5::OnCollision() {}

void EnemySkydome5::Draw() {
...
void EnemySkydome5::Fire() {
	...
	if (spawnTimer < -0.0f) {
		...
		// 弾を生成し、初期化
		EnemySkydomeBullet5* newBullet = new EnemySkydomeBullet5();
		newBullet->Initialize(modelbullet_, moveBullet, velocity);

		// 弾を登録する
		bullets5_.push_back(newBullet);

		spawnTimer = kFireInterval;
	}
}

Vector3 EnemySkydome5::GetWorldPosition() {
	// ワールド座標を入れる変数
	Vector3 worldPos;
