...
void EnemySkydome::Initialize(Model* model, ViewProjection* viewProjection, const Vector3& pos) 
{
	assert(model);
	model_ = model;
...
void EnemySkydome::OnCollision() {}

void EnemySkydome::Draw() 
{
...
void EnemySkydome::Fire() 
{
	...
	if (spawnTimer < -0.0f) {
		...
		// 弾を生成し、初期化
		EnemySkydomeBullet* newBullet = new EnemySkydomeBullet();
...
Vector3 EnemySkydome::GetWorldPosition() 
{
	// ワールド座標を入れる変数
	Vector3 worldPos;
